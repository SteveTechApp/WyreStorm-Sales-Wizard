...
// FIX: Added optional `disabled` prop to support disabled states, which was being passed from a parent component.
export function ToggleSwitch({ id, label, checked, onChange, icon, disabled }: { id?: string; label: string; checked: boolean; onChange: (v: boolean) => void; icon?: React.ReactNode; disabled?: boolean }) {
  ...
  return (
    <label htmlFor={inputId} className={`group flex items-center justify-between gap-3 rounded-md border border-zinc-700/60 bg-zinc-900/60 p-3 transition ${disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:border-zinc-500/60'}`}>
