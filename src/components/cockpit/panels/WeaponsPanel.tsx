...
import { CockpitRocket, CockpitAlertTriangle, CockpitLampCeiling } from '@/components/cockpit/Icons';
import { Panel } from '@/components/cockpit/Panel';
import { SafeSwitch } from '@/components/cockpit/controls/GuardedSwitch';
import { MomentaryButton } from '@/components/cockpit/controls/MomentaryButton';
...
import { DesignFeedbackItem, ManuallyAddedEquipment } from '@/types';
...
export function WeaponsPanel() {
  ...
  const handleAnalyze = async () => {
    ...
    const feedback = await analyzeProject(projectData, userProfile).catch((e) => {
      ...
      <Panel title="AI WINGMAN" icon={<CockpitRocket className="size-4" />} tone="danger">
        <div className="grid gap-4">
          <Tooltip content="Enable/Disable all AI-powered design actions.">
            <SafeSwitch id="ai-assist" label="AI ASSIST" isSafe={!aiAssist} onToggle={setAiAssist} danger />
          </Tooltip>
          ...
          <div className="rounded-md border border-red-500/40 bg-red-950/20 p-3 text-red-200">
            <div className="flex items-center gap-2 text-sm"><CockpitAlertTriangle className="size-4" /><span>AI ACTIONS REQUIRE AI ASSIST TO BE ARMED</span></div>
          </div>
        </div>
      </Panel>
