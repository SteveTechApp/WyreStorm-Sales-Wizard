...
import { FormData, ProjectData, IO_Device } from './types';
...
const ProjectBuilder: React.FC<ProjectBuilderProps> = ({ onSubmit, onSaveProject, initialData }) => {
  ...
  const handleRoomDataChange = (updatedRoomData: FormData) => {
    setProjectData(pd => ({
        ...pd,
  ...
  const handleSelectRoomTypeToAdd = async (roomType: string) => {
    ...
    try {
        ...
        const roomsToAdd = (Array.isArray(templateResult) ? templateResult : [templateResult]).map((roomTemplate: Omit<FormData, 'id'>) => {
